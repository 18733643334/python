...
import time
root = Tk()
root.title(str('与xxx聊天中'))
#发送按钮事件
def sendmessage():
  #在聊天内容上方加一行 显示发送人及发送时间
  msgcontent = str('我:') + time.strftime("%Y-%m-%d %H:%M:%S",time.localtime()) + '\n '
...
button_sendmsg = Button(frame_left_bottom, text=str('发送'), command=sendmessage)
#创建一个绿色的tag
